	...
	for (const file of commandFiles ) {
		...
		if(command.perms) {
			guild.commands.fetch().then(async result => {
				await result.find(cmdResult => cmdResult.name == command.data.name)?.permissions
				.set({ permissions: permissions_constructor(guild, command.perms) })
			}) 
		}

		command_listener(client, command.data.name, (interaction: CommandInteraction) => {
			...
			logger.log(`executed command \`/${command.data.name}\` ${args.map(arg => `\`${arg}\``).join(" ")}`, interaction.user.tag)
