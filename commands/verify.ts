...
export const run = async (interaction: CommandInteraction) => {
	...
	const villagerRole = interaction.guild?.roles.cache.find(
		(r) => r.name == "Probably Not Villager"
	)!;
	const memberRole = interaction.guild?.roles.cache.find(
		(r) => r.name == "Probably Not Member"
	)!;
	...
	const nick =
		interaction.options.getString("in-game-name") || member?.displayName!;
	...
	let messages;
	try {
		messages = interaction.channel?.messages.cache.filter(
	...
	if (!member.roles.highest.editable) {
		interaction.editReply(
			`Cannot verify a user with the role ${member.roles.highest}`
		);
	...
	await member?.roles
		.add(memberRole)
		.then(() => setTimeout(() => member.roles.remove(villagerRole), 2000))
		.catch((e) => logger.log(e));
