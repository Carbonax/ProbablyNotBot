...
export const run = async (
	...
	const timeoutPromise = new Promise<MessageReaction | PartialMessageReaction>(
		(resolve, reject) => setTimeout(reject, 15000)
	);
	const reactionPromise = new Promise<MessageReaction | PartialMessageReaction>(
		(resolve) => {
	...
	await Promise.race([timeoutPromise, reactionPromise])
		.then(async (reaction) => {
			await reaction.message.reactions.cache
				.find((result) => result == reaction)
				?.users.remove(interaction.user.id);
			await reaction.users.remove(client.user?.id);
			const result = await database.RoleMessage.findOneAndDelete({
